...
> 本节基于 **Go 1.23.11** 版本，介绍 Goroutine 栈的动态管理机制。

### TL;DR - 面试速查

- Goroutine 初始栈大小为 **2 KB**（大多数平台）
- 栈可以动态增长，上限约为 **1 GB**
- 栈缩容发生在 **GC 期间**，当栈使用率低于 1/4 时触发
- 栈操作涉及复制和指针调整，有一定开销
...
栈缩容是栈增长的逆过程，发生在 GC 期间：

```go
// runtime/stack.go 中的栈缩容逻辑 (概念示意)
func shrinkstack(gp *g) {
    ...
    // 如果使用率低于 1/4，且栈大于最小栈大小
    if used < total/4 && total > _FixedStack {
        // 缩小到当前大小的一半
        newsize := total / 2
...
│                                                             │
│  2. 栈使用率 < 25%                                          │
│     - used = stack.hi - sched.sp                            │
│     - 使用率 = used / (stack.hi - stack.lo)                 │
│                                                             │
│  3. 当前栈大于最小栈 (2KB)                                   │
│     - 不能缩小到比 _FixedStack 更小                         │
│                                                             │
│  4. Goroutine 不在系统调用中                                 │
│     - 系统调用期间栈不能被移动                               │
│                                                             │
...
// 指针调整示例
func adjustpointers(gp *g, adjinfo *adjustinfo) {
    // 遍历栈帧
    for frame := range stackframes(gp) {
        // 调整帧中的指针
        for ptr := range frame.pointers {
            if ptr >= old.lo && ptr < old.hi {
                // 指针指向旧栈，需要调整
                *ptr = *ptr - old.lo + new.lo
            }
        }
    }
}
...
│  │      scanstack(g)     // 扫描栈上的指针              │   │
│  │      shrinkstack(g)   // 检查是否需要缩容            │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  为什么在 GC 期间缩容:                                       │
│  1. goroutine 已经停止，可以安全操作栈                       │
│  2. 已经在扫描栈，顺便检查使用率                             │
│  3. 避免额外的 STW 开销                                     │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
// 2. 长期运行的 goroutine 注意栈膨胀
// 如果 goroutine 偶尔需要大栈，栈会增长但不会立即缩小

// 3. 监控栈内存使用
// 使用 runtime.Stack() 或 pprof 监控
```
...
> Go 1.20 引入预览版，Go 1.21 正式可用于生产环境。

### TL;DR - 面试速查

- PGO 使用 CPU profile 数据指导编译器优化
- 典型性能提升 **2-14%**
- 将 `default.pgo` 放在 main 包目录，`go build` 自动使用
- Profile 应来自生产环境，而非微基准测试

---

### 什么是 PGO？

Profile-Guided Optimization (PGO)，也称为 Feedback-Directed Optimization (FDO)，是一种编译器优化技术：
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. 热点函数内联 (Hot Function Inlining)                    │
│     ┌─────────────────────────────────────────────────────┐│
│     │  Profile 显示 foo() 被频繁调用                       ││
│     │  → 编译器更积极地内联 foo()                          ││
│     │  → 减少函数调用开销                                  ││
│     └─────────────────────────────────────────────────────┘│
│                                                             │
...
│     ┌─────────────────────────────────────────────────────┐│
│     │  Profile 显示接口方法 90% 调用同一实现               ││
│     │  → 编译器生成特化代码                                ││
│     │  → 减少间接调用开销                                  ││
│     └─────────────────────────────────────────────────────┘│
│                                                             │
│  3. 基本块布局优化 (Basic Block Layout)                     │
│     ┌─────────────────────────────────────────────────────┐│
│     │  Profile 显示分支预测信息                            ││
│     │  → 优化代码布局，热路径连续                          ││
│     │  → 提高指令缓存命中率                                ││
...
```bash
# 方式 1: 自动检测 (推荐)
# 将 profile 重命名为 default.pgo 放在 main 包目录
...
```bash
# 查看编译时是否使用了 PGO
go build -pgo=auto -x ./cmd/myapp 2>&1 | grep pgo
...
│                                                             │
│  3. Profile 合并                                            │
│     # 合并多个 profile 获得更全面的数据                     │
│     go tool pprof -proto a.pprof b.pprof > merged.pprof    │
│                                                             │
│  4. CI/CD 集成                                              │
│     - 将 default.pgo 提交到代码仓库                         │
│     - 定期从生产环境更新                                    │
│     - 构建时自动使用                                        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
func (h *ConcreteHandler) Handle(ctx context.Context) error {
    // 实现
    return nil
}
...
func process(h Handler) {
    // 无 PGO: 每次都是间接调用
    // 有 PGO: 如果 profile 显示 90% 是 ConcreteHandler
    //        编译器会生成:
    //        if h is *ConcreteHandler {
    //            直接调用 ConcreteHandler.Handle
    //        } else {
    //            间接调用
    //        }
...
> Profile-Guided Optimization，使用程序运行时的 CPU profile 数据来指导编译器做出更好的优化决策，如更积极的内联、去虚拟化等。

**Q2: 如何使用 PGO？**

> 1. 从生产环境收集 CPU profile
> 2. 将 profile 命名为 `default.pgo` 放在 main 包目录
> 3. 运行 `go build`，编译器会自动使用

**Q3: PGO 能带来多少性能提升？**

> 根据官方数据，典型提升在 2-14% 之间，具体取决于应用特性。

**Q4: 为什么 profile 应该来自生产环境？**

> 生产环境的 profile 反映真实的代码执行路径和热点，编译器据此优化能获得最大收益。微基准测试可能不代表真实使用模式。
...
- [PGO 设计提案](https://go.googlesource.com/proposal/+/master/design/55022-pgo-implementation.md)
- [Go 1.21 PGO 博客](https://go.dev/blog/pgo)
